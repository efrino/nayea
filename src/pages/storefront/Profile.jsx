    ...
    return (
      <div className="bg-gray-50 min-h-screen py-12 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-4xl sm:text-5xl font-black font-heading text-gray-900 tracking-tighter italic uppercase mb-10">Profil Saya</h1>
          <div className="bg-white rounded-[3rem] border border-gray-100 shadow-sm p-12 text-center">
            ...
            <h2 className="text-2xl font-black font-heading text-gray-900 italic uppercase mb-2">Login untuk melihat profil</h2>
            <p className="text-gray-500 font-medium mb-8">Daftar atau masuk ke akun untuk melihat riwayat pesanan dan alamat tersimpan Anda.</p>
            <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="lg:grid lg:grid-cols-12 lg:gap-x-10 lg:items-start">
          ...
          <div className="lg:col-span-3 mb-8 lg:mb-0">
            <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-6 space-y-1">
              {TABS.map((tab) => {
                const Icon = tab.icon;
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-5 py-4 rounded-[1.5rem] text-[11px] font-black uppercase tracking-widest transition-all ${
                      isActive ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900'
                    }`}
                  >
  ...
  return (
    <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-8 sm:p-10">
      <div className="flex items-center gap-5 mb-10 pb-10 border-b border-gray-50">
        ...
        <div>
          <h3 className="text-xl font-black font-heading text-gray-900 uppercase italic tracking-tight">{fullName}</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div className="p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Nama Lengkap</p>
        ...
        <div className="p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Bergabung Sejak</p>
          <p className="text-sm font-black text-gray-900">{joinDate}</p>
        </div>
        <div className="p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Status Akun</p>
  ...
  if (orders.length === 0) {
    return (
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-16 text-center">
        ...
        <h3 className="text-xl font-black font-heading text-gray-900 italic uppercase mb-2">Belum ada pesanan</h3>
        <p className="text-gray-500 font-medium mb-8">Riwayat belanja Anda akan muncul di sini setelah checkout pertama.</p>
        ...
        <div key={order.id} className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-8">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-6 border-b border-gray-50">
            <div>
              ...
              <p className="text-lg font-black text-gray-900 italic">Rp {Number(order.total_amount).toLocaleString('id-ID')}</p>
            </div>
            ...
            {(order.order_items || []).map((item, idx) => (
              <div key={idx} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-gray-800 truncate">{item.product?.name || 'Produk tidak tersedia'}</p>
                  <p className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">{item.quantity}x &middot; Rp {Number(item.price).toLocaleString('id-ID')}</p>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="space-y-6">
      ...
      {addresses.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-16 text-center">
          ...
          <h3 className="text-xl font-black font-heading text-gray-900 italic uppercase mb-2">Belum ada alamat tersimpan</h3>
          <p className="text-gray-500 font-medium">Tambahkan alamat supaya checkout jadi lebih cepat lain kali.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {addresses.map((addr) => (
            <div key={addr.id} className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-7 relative">
              {addr.is_default && (
              ...
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">{addr.label}</p>
              <h4 className="text-base font-black text-gray-900 mb-1">{addr.recipient_name}</h4>
              <p className="text-xs text-gray-400 font-bold mb-3">{addr.phone}</p>
              <p className="text-sm text-gray-500 font-medium leading-relaxed mb-6">{addr.full_address}</p>

              <div className="flex items-center gap-2">
                {!addr.is_default && (
                  <button
                    onClick={() => handleSetDefault(addr.id)}
                    className="flex-1 py-3 rounded-[1.2rem] bg-gray-50 text-gray-500 text-[10px] font-black uppercase tracking-widest hover:bg-gray-100 transition-all"
                  >
                    Jadikan Utama
                  </button>
                )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl p-8 sm:p-10">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-xl font-black font-heading text-gray-900 uppercase italic tracking-tight">
                {editId ? 'Edit Alamat' : 'Alamat Baru'}
              </h3>
              <button onClick={closeModal} className="p-2 rounded-full bg-gray-50 text-gray-400 hover:text-gray-900 transition-all">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Label Alamat</label>
                <input
                  type="text" required value={form.label}
                  onChange={(e) => setForm({ ...form, label: e.target.value })}
                  placeholder="Rumah, Kantor, dll"
                  className="w-full px-6 py-4 rounded-[1.2rem] bg-gray-50 border-transparent focus:bg-white focus:ring-2 focus:ring-primary outline-none text-sm font-medium transition-all"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-6 py-4 rounded-[1.2rem] bg-gray-50 border-transparent focus:bg-white focus:ring-2 focus:ring-primary outline-none text-sm font-medium transition-all resize-none"
                />
              </div>

              <div className="pt-4 flex gap-4">
                <button type="button" onClick={closeModal} className="flex-1 py-4 rounded-[1.5rem] bg-gray-50 text-gray-500 text-xs font-black uppercase tracking-widest hover:bg-gray-100 transition-all">
                  Batal
                </button>
                <button
